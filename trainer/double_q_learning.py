...
class DoubleQLearning(QLearning):
    ...
    def calc_mean_loss(self, s: torch.FloatTensor, a: torch.BoolTensor, r: torch.FloatTensor, done: torch.BoolTensor, next_s: torch.FloatTensor) -> torch.FloatTensor:
        ...
        with torch.no_grad():
            ...
            best_next_action_onehot = torch.nn.functional.one_hot(best_next_action_ind, self.config.env_action_space.count[0])
            q_target = r + self.config.env_discount_rate * torch.bitwise_not(done) * torch.sum(self.target_q_net(next_s) * best_next_action_onehot, dim = -1)
